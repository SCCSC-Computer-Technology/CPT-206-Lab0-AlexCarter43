...
namespace ConsoleApp1CSharp
{
    class Program
    {
        ...
        private static void SelectionSort(int[] arrayToSort)//1000
        {
            int minIndex = 0;
            for(int i = 0;i < arrayToSort.Length-2;i++)
            {
                ...
                for(int j = i+1; j< arrayToSort.Length-1; j++)
                {
                    ...
                    arrayToSort[minIndex] = arrayToSort[j];
                    arrayToSort[j] = temp;
                }
                
            }
        }
        private static void newArrayStuff()
        {
            //create an array
            int[] numbers = new int[50];

            //fill the array with random numbers
            Random newNumber = new Random();//creates a random object
            for(int i=0; i<numbers.Length; i++)
            {
                numbers[i] = newNumber.Next(501);//assign random numbers to our array elements 0-end
            }
            //print the array
            ...
            for (int i= numbers.Length-1; i>=0;i--)
            {
                ...
                if (numbers[i] > 400)
                {
                    Console.Write("\tVery Large Number");
                }
                Console.Write("\n");
            }
            ...
            for (int i = 0; i < numbers.Length; i++)
            {
                ...
                if (numbers[i] > 400)
                {
                    Console.WriteLine(numbers[i]+"\tVery Large Number");
                }
                else
                {
    ...
    class CPT244Node<T1>
    {
        //Fields or poperties of our cpt 244 node with a generic type (T1)
        public T1 OurData {  get; set; }
        public CPT244Node<T1> Next244Node { get; set; }

        //constructor
        public CPT244Node(T1 ourData)//parameter
        {
            OurData = ourData;
            Next244Node = null;
        }
    ...
    class CPT244LinkedListStack<T1>
    {
        //initial head
        private CPT244Node<T1> stackHead;

        //push
        public void Push(T1 ourData)
        {
            CPT244Node<T1> CPT244Node = new CPT244Node<T1>(ourData);
            CPT244Node.Next244Node = stackHead;
            stackHead = CPT244Node;

        }
        //pop
        public void Pop(T1 ourData)
        {
            if(isEmpty())
            {
                Console.WriteLine("stack empty nothing to remove");

            }
            else
            {
                //remove it
                Console.WriteLine("Pop: " +  stackHead);
                stackHead = stackHead.Next244Node;

            }
        }
        //peek
        public CPT244Node<T1> Peek()
        {
            return  stackHead;
        }
        //isEmpty-bool
    ...
    //python: dictionary
    //element: key, value; value is your data = > key is the pointer to the next
    //value = data, key = next
        ...
        public void Add(T data)//Big O(n)< O 2(n)^2 < O(2^n) < O(n!)=permutation
        {
            ...
            else//head is not null
            {
                ...
                while (current.Next != null)//n
                {
                    ...
                    current = current.Next;//traversing the list; moving the pointer
